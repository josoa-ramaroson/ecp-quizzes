...
import { DailyQuizCompletionDto, GetMetricsDto, LeaderBoardDto, MonthlyQuizCompletionDto, PersonalStatsDto, TDayDto, TMonthlyNameDto } from './dto';
...
export class StatsService {
  ...
  async getWeeklyMemberLeaderboard() {
    ...
    const stats = await this.getLeaderboardFromDate(lastLastMonday, EMemberRole.MEMBER);
    return stats;
  }
  ...
  async getTopPerformerMember() {
    ...
    return (await this.getLeaderboardFromDate(statQuiz, EMemberRole.MEMBER)).slice(0, 3);
  }

  async getLeaderboardFromDate(date: Date, role: EMemberRole): Promise<LeaderBoardDto[]> {
  ...
  async getWeeklyCompletion(): Promise<DailyQuizCompletionDto[]> {
    const days: TDayDto[]  = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"];
  ...
  async getAverageScoreOfMember(memberId: string | Types.ObjectId): Promise<number> {
    try {
      ...
      const scorePercentages = await Promise.all(
        answerHistory.map(async history => {
          try {
            ...
            console.error(`Error calculating score for quiz ${history.quizId}: ${error.message}`);
      ...
      const averageScore = scorePercentages.length > 0 
        ? scorePercentages.reduce((sum, percentage) => sum + percentage, 0) / scorePercentages.length 
        : 0;

      return Math.round(averageScore * 10000) / 100;
    } catch (error) {
      console.error(`Error calculating average score for member ${memberId}: ${error.message}`);
      return 0; // Return 0 on error
    }
  }

  async getPerfectScoreCount(memberId: string | Types.ObjectId): Promise<number> {
    try {
      const answerHistory = await this.answerHistoryService.findByMember(memberId.toString());
      ...
      const perfectScores = await Promise.all(
        answerHistory.map(async history => {
          try {
            ...
            console.error(`Error checking perfect score for quiz ${history.quizId}: ${error.message}`);
      ...
      console.error(`Error calculating perfect scores for member ${memberId}: ${error.message}`);
      return 0; // Return 0 on error
    }
  }

  async getTotalQuestionsAnswered(memberId: string | Types.ObjectId): Promise<number> {
    try {
      const answerHistory = await this.answerHistoryService.findByMember(memberId.toString());
      ...
      const questionsAnswered = await Promise.all(
        answerHistory.map(async history => {
          try {
            // Get the number of questions in this quiz from the answers record
            return history.answersRecord?.length || 0;
          } catch (error) {
            console.error(`Error counting questions for quiz ${history.quizId}: ${error.message}`);
            return 0; // Don't count on error
          }
        })
      );
      ...
      console.error(`Error calculating total questions for member ${memberId}: ${error.message}`);
      return 0; // Return 0 on error
    }
  }
}
